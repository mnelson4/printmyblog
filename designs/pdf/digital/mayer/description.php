...
<p>
    <?php esc_html_e('Designed to look good when viewed from a device or printing to paper. The two-column layout leads to more compact content, especially good if your content doesn’t require the full page width', 'print-my-blog');?><?php pmb_pro_best_e(__('Most browsers will only print a white background', 'print-my-blog'));?>
</p>

<h2><?php esc_html_e('Features', 'print-my-blog');?></h2>
<ul class="pmb-list">
    <li><?php esc_html_e('Main matter is divided into two columns, while front matter uses the full page width.', 'print-my-blog');?><?php pmb_pro_best_e(__('Some browsers don‘t support multiple columns', 'print-my-blog'));?></li>
    ...
    <li><?php esc_html_e('Internal links (hyperlinks to content included in the project) are replaced with page references.',
            'print-my-blog');?><?php pmb_pro_only_e();?></li>
...
<ul class="pmb-list">
     <li><?php esc_html_e('Page numbers in bottom-right corner, and part titles in the top-left. Front-matter’s pages are numbered with Roman numerals', 'print-my-blog');?><?php pmb_pro_best_e(__('Most browsers add their own content to the page margins', 'print-my-blog'));?></li>
</ul>
